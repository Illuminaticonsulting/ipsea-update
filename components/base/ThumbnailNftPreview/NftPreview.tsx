...
const ThumbnailNftPreview = ({
  ...
  if (thumbnailNFT === null) {
    return (
      <NftUpload
        className={className}
        content="Click here to upload your Thumbnail IP."
        inputId="uploadThumbnailNft"
        note={`File Max Size 30mb.`}
  ...
  return (
    <div className={className}>
            ...
            <NftUpload
              content={
                <>
                  ...
                  <span>{thumbnailNFT.name}</span>
                </>
              }
              inputId="reUploadThumbnailNft"
              isMinimal
              onChange={handleFileUpload}
            />
          </SReuploadWrapper>
      ...
      <SFieldset>
        {NFT_EFFECTS_ORDERED.filter((effectType) => handleAllowedEffect(thumbnailNFT, effectType)).map((effectType) => (
...
export default React.memo(ThumbnailNftPreview)
