...
const ModalMint: React.FC<ModalProps> = ({
  ...
  const handleMintSocketProcess = () => {
    ...
    socket.once('CONNECTION_SUCCESS', () => {
      if (isRN || window.isRNApp) {
        ...
        const fetchData = async () => {
          ...
          try {
            const { categories, nftUrls, seriesId } = await uploadNFT([session], setProgressData)
    ...
    socket.once('MINTING_NFT_ERROR', ({ reason }: { success: boolean; reason: string }) => {
      ...
      if (reason === 'fees') {
        setError(
          'Process was cancelled, please check your account has enough tokens to pay for the transaction and try again.'
        )
      } else {
